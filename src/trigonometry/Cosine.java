...
/**
 * Utility class that provides methods to calculate the trigonometric cosine function for radians and degrees input.
...
public class Cosine
{
    ...
    /**
     * This method calculates the cosine function from a radian input. The cosine function is based on the
     * Taylor expansion series, with limits from -2*pi to 2*pi.
     *
     * @param x the radian input
     * @return the value of cosine for that input
     */
    public static double calculateCosineFromRadianInput (double x)
    {
        if (x > 2 * PI || x < -2 * PI)
        {
    ...
    /**
     * This method calculates the cosine function from a degree input. The cosine function is based on the
     * Taylor expansion series, with limits from -2*pi to 2*pi.
     *
     * @param inDegrees the degree input
     * @return the value of cosine for that input
     */
    public static double calculateCosineFromDegreesInput (double inDegrees)
    {
        // Validate -360 to 360
        double inRadian = (inDegrees * (Math.PI / 180.0));
        return calculateCosineFromRadianInput(inRadian);
    }
}
